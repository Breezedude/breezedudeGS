...
const WS_TIMEOUT_MS = 10000; // 10 seconds without message = disconnected
...
function saveWiFi() {
    const data = {
        sta_ssid: getVal("sta_ssid"),
        sta_password: getVal("sta_password"),
        ap_ssid: getVal("ap_ssid"),
        ap_password: getVal("ap_password"),
        keepAP: getVal("keepAP")
    };
...
function saveSettings() {
    const data = {
        name: getVal("deviceName"),
        lon: parseFloat(getVal("lon")),
        lat: parseFloat(getVal("lat")),
        elevation: parseInt(getVal("elevation")),
        aprsServer: getVal("aprsServer"),
        aprsPort: parseInt(getVal("aprsPort")), 
        sendBreezedude: getVal("sendBreezedude")
    };
...
window.addEventListener('load', () => {
  //document.getElementById("getLocationBtn").addEventListener("click", getUserLocation);
    onWsReady(() => {
        ...
        loadFields();
        wsSend({ cmd: "get_fanet"});

        setInterval(() => {wsSend({ cmd: "get_info_update"});}, 1000);
    });
  ...
  ws.onmessage = function (event) {
    ...
    if(data.error){
      ...
      data.weather.forEach(station => {
          const rowData = {
              id: (station.vid.toString(16).toUpperCase() + station.fanet_id.toString(16).toUpperCase()),
              name: station.name,
              ...
              lastSeen: station.tLastMsg !== undefined ? station.tLastMsg : "-"
          };
      ...
      data.tracking.forEach(trk => {
          const rowData = {
              id: (trk.vid.toString(16).toUpperCase() + trk.fanet_id.toString(16).toUpperCase()),
              name: trk.name,
              ...
              lastSeen: trk.tLastMsg !== undefined ? trk.tLastMsg : "-",
              state: "-"
          };
